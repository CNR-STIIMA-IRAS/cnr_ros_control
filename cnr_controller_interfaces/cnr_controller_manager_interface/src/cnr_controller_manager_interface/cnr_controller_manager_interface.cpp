                                         ...
                                         , const ros::Duration&                                          watchdog )
{
  ...
  if( _ctrl_to_load_and_start_names  ) std::transform(ctrl_to_load_and_start_names  ->begin(), ctrl_to_load_and_start_names  ->end(), _ctrl_to_load_and_start_names  ->begin(), []( auto v ) { return v.name; });
  if( _ctrl_to_restart_names         ) std::transform(ctrl_to_restart_names         ->begin(), ctrl_to_restart_names         ->end(), _ctrl_to_restart_names         ->begin(), []( auto v ) { return v.name; });
  if( _ctrl_to_stop_unload_names ) std::transform(ctrl_to_stop_unload_names ->begin(), ctrl_to_stop_unload_names ->end(), _ctrl_to_stop_unload_names ->begin(), []( auto v ) { return v.name; });
...
bool ControllerManagerInterface::unload_ctrl( const std::string& ctrl_to_unload_name, const ros::Duration& watchdog)
{
  ...
  else if (!unload_ctrl_srv.response.ok)
  {
    ...
    ret = false; CNR_RETURN_FALSE(*logger_ );
  }
...
bool ControllerManagerInterface::unload_ctrl( const std::vector<controller_manager_msgs::ControllerState>& ctrl_to_unload_names, const ros::Duration& watchdog)
{
  ...
  std::transform(ctrl_to_unload_names.begin(), ctrl_to_unload_names.end(), _ctrl_to_unload_names.begin(), []( auto v ) { return v.name; });
                          ...
                          , const ros::Duration&            watchdog)
{
  ...
  bool ok = true;
  std::for_each(unload_ok.begin(), unload_ok.end(), [&]( std::pair<std::string,bool> b) { ok &= b.second; });
