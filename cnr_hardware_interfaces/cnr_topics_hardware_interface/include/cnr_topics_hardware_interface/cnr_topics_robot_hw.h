...
: cnr_hardware_interface::ClaimedResource< geometry_msgs::WrenchStamped > ( wr, {wr.m_sensor_name}, robot_hw_nh, topics_received )
{
...
: cnr_hardware_interface::ClaimedResource< geometry_msgs::PoseStamped > ( pr, {pr.m_frame_id}, robot_hw_nh, topics_received )
{
  size_t l = __LINE__;
  try
  {
...
#define ll l = __LINE__;
{
  ...
  try
  {
    ...
    m_state.resize(pr.m_subscribed_topics.size(), geometry_msgs::TwistStamped() );  ll;
    for( size_t i=0; i<m_state.size(); i++ )
    {
      m_state.at (i).header.frame_id = pr.m_frames_id.at(i);
    }

    m_output.resize(pr.m_published_topics.size(), geometry_msgs::TwistStamped() );  ll;
